...
from tasks.vqa_model_v2 import FusionAfterCrossVQAModel, MultiLoss, JointVQAModel, LSTMCNNCrossVQAModel, LSTMCNNFusionBeforeCrossVQAModel, LSTMCNNFusionBothCrossVQAModel
from tasks.vqa_data_v2 import VQADataset, VQATorchDataset, VQAEvaluator
...
# I just hardcode this.
ans2label = {"binary": 0, "number": 1, "unanswerable": 2, "other": 3, "color": 4}
...
def get_data_tuple(splits: str, bs: int, shuffle=False, drop_last=False, config=None) -> DataTuple:
    print(splits)
    dset = VQADataset(splits, config=config)
...
class VQA:
    def __init__(self, config=None):
        ...
        if args.valid != "":
            self.valid_tuple = get_data_tuple(
                args.valid, bs=1024,
                shuffle=False, drop_last=False, config=config
            )
        else:
            self.valid_tuple = None

        # Model
        # self.model = LSTMCNNCrossVQAModel(config, self.train_tuple.dataset.num_answers)
        # self.model = LSTMCNNFusionBeforeCrossVQAModel(config, self.train_tuple.dataset.num_answers)
        self.model = LSTMCNNFusionBothCrossVQAModel(config, self.train_tuple.dataset.num_answers)

        # @TODO avoid magic number
        # self.model = JointVQAModel(self.train_tuple.dataset.num_answers, 5)
        ...
        # Load finetuned Classifier's weight
        if args.load_cls is not None:
            print("Loading Model from %s" % args.load_cls)
            log = torch.load(args.load_cls)
            dict_weights = log['weights']

            # we don't need dict for last layer
            partial_dict = {k: v for k, v in dict_weights.items() if k not in ["module.classifier.lin2.weight", "module.classifier.lin2.bias"]}
            self.model.classifier.load_state_dict(partial_dict)

        print("classifier loaded from checkpoint")
        ...
        # Loss and Optimizer
        # @TODO
        self.loss = MultiLoss(args.alpha, args.beta)
        ...
        # Add shceduler, magic number, change to args
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optim, factor=0.7, patience=2, verbose=True,
                                                              mode='max')
        # Output Directory
    ...
    def train(self, train_tuple, eval_tuple):
        ...
        for epoch in range(args.epochs):
            quesid2ans = {}
            for i, (item, ques_id, feats, boxes, sent, target, quesType) in iter_wrapper(enumerate(loader)):

                self.model.train()
                self.optim.zero_grad()
                quesType = torch.tensor(np.array(list(map(lambda x: ans2label[x], quesType))))
                feats, boxes, target, quesType = feats.cuda(), boxes.cuda(), target.cuda(), quesType.cuda()
                vqa_logit, cls_logit = self.model(item, feats, boxes, sent)
                # assert logit.dim() == target.dim() == 2
                loss = self.loss(cls_logit, vqa_logit, quesType, target)

                loss.backward()
                nn.utils.clip_grad_norm_(self.model.parameters(), 5.)
                self.optim.step()

                score, label = vqa_logit.max(1)
                for qid, l in zip(ques_id, label.cpu().numpy()):
                    ans = dset.label2ans[l]
                    quesid2ans[qid] = ans

            acc = evaluator.evaluate(quesid2ans) * 100.
    ...
    def predict(self, eval_tuple: DataTuple, dump=None):
        ...
        for i, datum_tuple in enumerate(loader):
            ...
            item, ques_id, feats, boxes, sent, target, quesType = datum_tuple
            with torch.no_grad():
                feats, boxes = feats.cuda(), boxes.cuda()
                logit = self.model(item, feats, boxes, sent)[0]
    ...
    def oracle_score(data_tuple):
        ...
        quesid2ans = {}
        for i, (_, ques_id, feats, boxes, sent, target, quesType) in enumerate(loader):
            print(i)
...
if __name__ == "__main__":
    ...
    if args.lstmcnn_path_config is not None:
        with open(args.lstmcnn_path_config, 'r') as handle:
